...
5. Log all activity to console and `data/logs/` directory
...
Logs are saved in `data/logs/` directory:

```bash
# View latest log
cat data/logs/bot-$(date +%Y-%m-%d).log

# Windows PowerShell
Get-Content data\logs\bot-$(Get-Date -Format "yyyy-MM-dd").log
```
